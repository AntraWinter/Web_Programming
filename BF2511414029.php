...
<html>
    ...
    <body>
        ...
        <td width="300px">
                <?php
                    $conn=new mysqli("localhost", "root", "","befree-website");
                    if ($conn->connect_error) {
                        die("Ошибка подключения: " . $conn->connect_error);
                    }
                    $sql = "SELECT * FROM images WHERE article = 'BF2511414029'";
            ...
            <td width="500px">
                <div>
                    ...
                    $sql = "SELECT product.article, product.product_name, product.price, description.color, description.composition, description.cutout, description.straps, description.length, description.sleeve, description.waist, description.recommendations
                    FROM product JOIN description ON product.article=description.article
                    WHERE product.article = 'BF2511414029'";
                    $result = $conn->query($sql);
                    // Проверка результата
                    if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            <form action="Php/add_to_cart.php" method="post">
                                <p>выберите размер:
                                <input type="hidden" name="product_id" value="BF2511414029">
                            ...
                            <p><b>состав и параметры</b><br>
                                ...
                                длина: '.$row["length"].'<br>
                                вырез: '.$row["cutout"].'<br>
                                вид бретелей: '.$row["straps"].'<br>
                                рукава: '.$row["sleeve"].'<br>
                                талия: '.$row["waist"].'<br>
                            </p>
